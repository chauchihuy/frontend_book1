...
export class InsertProductAdminComponent implements OnInit {
  ...
  insertProduct() {
    ...
    if (this.insertEbookDTO.name.length === 0) {
      alert("Tên ebook không để trống");
    }
    if (this.insertEbookDTO.title.length === 0) {
      alert("Title ebook không để trống");
    }
    if (this.insertEbookDTO.document.length === 0) {
  ...
  uploadImages(ebookId: number) {
    ...
    this.ebookService.uploadImages(ebookId, this.insertEbookDTO.images).subscribe({
      next: (imageResponse) => {
        // console.log('Hình ảnh đã được tải lên thành công:', imageResponse);
        // alert('Hình ảnh đã được tải lên thành công:');
      },
      error: (error) => {
        console.error('Lỗi khi tải hình ảnh lên:', error);
        alert(`Lỗi khi tải hình ảnh lên: ${error.error?.message || error.message}`);
      }
    });
  }
  ...
  uploadMp3s(ebookId: number) {
    ...
    this.ebookService.uploadmp3s(ebookId, this.insertEbookDTO.mp3s).subscribe({
      next: (audioResponse) => {
        ...
        // alert('Đã tải âm thanh thành công');
      },
      // error: (error) => {
