  ...
  placeOrder() {
    ...
    // }
      ...
      if (this.orderForm.errors == null) {
        ...
        const unavailableEbook = this.cartItems.find(cartItem => cartItem.ebook.active === true);
        if (unavailableEbook) {
          alert('Có Ebook ' + unavailableEbook.ebook.name + ' ngừng bán. Không thể mua ngay bây giờ');
          return;
        }
