...
export class OrderUserComponent implements OnInit {
  ...
  searchOrders() {
    ...
    let searchKeyword = this.keyword.trim();
    switch (searchKeyword) {
      case 'Hoàn Thành':
        searchKeyword = 'COMPLETE';
        break;
      case 'Đang xử lý':
        searchKeyword = 'PROCESSING';
        break;
      case 'Đã Hủy':
        searchKeyword = 'CANCEL';
        break;
      case 'Tạm khóa':
        searchKeyword = 'false'; // Chuyển thành false để tìm kiếm
        break;
      case 'Đang mở':
        searchKeyword = 'true'; // Chuyển thành true để tìm kiếm
        break;
      default:
        // Giữ nguyên từ khóa nếu không khớp với bất kỳ trường hợp nào
    ...
    // }
    if (userId && !isNaN(userId)) {
      this.getOrdersByUserId(userId, searchKeyword, this.currentPage, this.itemsPerPage);
    }
  }
  ...
  getOrdersByUserId(userId: number, keyword: string, page: number, limit: number) {
    this.orderService.getAllOrdersByUserId(userId, keyword, page, limit).subscribe({
      next: (response: any) => {
        // Kiểm tra cấu trúc của response để lấy đúng dữ liệu
        this.orders = response.content || []; // Lấy dữ liệu từ response.content
        this.totalPages = response.totalPages || 1; // Gán mặc định nếu không có totalPages
